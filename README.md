...
- **Verdict-first reviews** — every review opens with a clear merge verdict and the exact blockers/warnings behind it, a per-file "What changed" breakdown grounded in the diff, and a Mermaid diagram of how the pieces relate. It publishes a GitHub check run (so Mira appears in the PR's checks and can be a required status check), and links a Linear ticket to grade every acceptance criterion against the diff — miss one and the PR gets a blocking "Request changes" review.
