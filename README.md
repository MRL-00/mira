...
- **Verdict-first reviews** — every review opens with a clear merge verdict and the exact blockers/warnings behind it, a per-file "What changed" breakdown grounded in the diff, and a Mermaid diagram of how the pieces relate. Link a Linear ticket and Mira grades every acceptance criterion against the diff — miss one and the PR gets a blocking "Request changes" review.
