...
class TestPostReviewRequestChanges:
    """A blocking verdict posts the review as REQUEST_CHANGES on GitHub."""

    def _provider(self, create_review=None, create_review_comment=None):
        provider = GitHubProvider.__new__(GitHubProvider)
        provider._token = "test-token"
        mock_commit = MagicMock()
        mock_pr = MagicMock()
        ...
        if create_review is not None:
            mock_pr.create_review.side_effect = create_review
        if create_review_comment is not None:
            mock_pr.create_review_comment.side_effect = create_review_comment
        mock_repo = MagicMock()
        mock_repo.get_pull.return_value = mock_pr
        mock_gh = MagicMock()
        mock_gh.get_repo.return_value = mock_repo
        provider._github = mock_gh
        return provider, mock_pr

    def _comment(self) -> ReviewComment:
    ...
    @pytest.mark.asyncio
    async def test_request_changes_event_when_flagged(self):
        calls: list[dict] = []
    ...
    @pytest.mark.asyncio
    async def test_summary_fallback_retries_as_comment(self):
        """If REQUEST_CHANGES is rejected, the summary still posts as a comment."""
        gh_422 = GithubException(status=422, data={"errors": ["cannot request changes"]})
        calls: list[dict] = []
        ...
        def _create_review(**kwargs):
            ...
            if kwargs.get("comments"):
                raise gh_422
            # Empty-comments calls: reject REQUEST_CHANGES, accept COMMENT.
            if kwargs.get("event") == "REQUEST_CHANGES":
                raise gh_422
            return MagicMock()

        provider, _ = self._provider(
            create_review=_create_review,
            create_review_comment=lambda **kw: (_ for _ in ()).throw(gh_422),
        )
        result = ReviewResult(comments=[self._comment()], summary="Found issues")
        await provider.post_review(_make_pr_info(), result, request_changes=True)
        events = [c.get("event") for c in calls if not c.get("comments")]
