...
- **Ticket acceptance-criteria verification.** When a PR links a ticket, Mira pulls the full ticket — description, acceptance criteria, labels, priority, comments, and sub-issues — grades *every* criterion against the PR diff, and renders a per-criterion checklist in the walkthrough. A requirement the model skips is shown as `unclear`, never assumed met. The grade is **advisory**: an `unmet` criterion downgrades the verdict to "Needs review" and is listed under the verdict for the human reviewer to check, but never blocks the merge on its own — only code blockers do. Tickets already in a completed/canceled state (e.g. a related epic that shipped) are not graded, ticket comments are background context rather than gradable requirements, and code outside the diff is graded `unclear` rather than `unmet`. The grading call can run on its own model via `llm.ticket_model` / `llm.ticket_reasoning_effort` (defaults to the review tier).
- **Visible re-reviews.** `@mira review` now reacts to the command comment (👀 on pickup, 🚀 on completion, 😕 on failure or when a review is already running), and the walkthrough footer stamps the reviewed commit and time, so a re-review that edits the walkthrough in place and lands the same verdict no longer looks like nothing happened.
- **Code-grounded thread replies.** When a developer replies to one of Mira's inline comments — "why is this a problem?", "that's handled in X", "how would you fix it?" — Mira now goes back to the code before answering. The reviewer model gets the PR title/description, the diff for the file under discussion, and `read_file` / `grep_repo` over the PR head (up to 5 hops), and replies with specifics (`path:line`) instead of paraphrasing its earlier comment. A disagreement is only conceded — thread resolved, `rejected` signal recorded — when the model *verifies* the developer's claim in the code; an unverified one gets a reply explaining what was found and stays open, so a blocker can't be waved away.
- **Replies without an @-mention.** A developer replying inside one of Mira's inline threads on GitHub no longer has to tag the bot — Mira checks that the thread root is its own and answers (or resolves) the reply the same way it does a mentioned one. Replies in human-to-human threads are ignored.
- **Blocking "Request changes" reviews.** A "Request changes" verdict (a code blocker) now posts the platform review as `REQUEST_CHANGES` (GitHub) instead of a plain comment, so the PR is actually blocked. A verdict with no inline findings still posts, and if the host rejects `REQUEST_CHANGES` the summary falls back to a comment. Opt out with `review.request_changes_on_blocker: false`.
- **GitHub check runs.** Each review now publishes a completed GitHub check run (default name `Mira Review`, configurable via `review.check_name`) so Mira appears in the PR's checks list and can be made a required branch-protection status check. The conclusion maps to the verdict — `failure` for Request changes, `neutral` for Needs review, `success` otherwise — and the check output carries the ticket criteria checklist and filed findings. Requires the GitHub App's **Checks: read and write** permission; without it the check run is skipped with a warning and the review is unaffected. Disable with `review.check_run: false`.
...
- **An open blocker can no longer be cleared by a re-review.** Mira is told not to repeat a finding it already posted, and `drop_already_posted` removes overlaps, so a fresh run that found nothing new could report `Looks good to merge` while its own blocker thread was still unresolved — the verdict only looked at *new* comments. Still-open threads are now rebuilt as findings (severity and title recovered from the posted comment body) and feed the verdict, the confidence clamp, the walkthrough (announced as "N findings below are still open from an earlier review"), the check run, and the blocking review body. Outdated or unlocatable threads are ignored, and threads the verifier resolves as fixed drop out as before.
